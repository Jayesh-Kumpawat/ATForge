...
def _mock_sma_llm(fast: int = 10, slow: int = 25):
    ...
    def _router(req: LlmRequest) -> LlmResponse:
        return LlmResponse(text=resp, model="mock", provider="mock", input_tokens=5, output_tokens=10, latency_ms=1)
...
def _seed_gen0(db_path, run_id, tmp_path):
    ...
    with connect(db_path) as conn, txn(conn):
        ...
        sid = upsert_strategy(
            conn, name="SMA_5x15_bullish", family="indicator",
        ...
        pd.DataFrame(
            {"open": [100.0]*50, "high": [101.0]*50, "low": [99.0]*50,
             "close": [100.0 + i*0.1 for i in range(50)], "volume": [10_000.0]*50},
            index=idx,
        ).to_parquet(ohlcv_path)
        sig_id = insert_pattern_signal(
            conn, run_id=run_id, strategy_id=sid, symbol="RELIANCE",
            n_signals=5, first_date=None, last_date=None, generation=0,
        )
        bt = BacktestResult(
            symbol="RELIANCE", pattern_name="SMA_5x15_bullish",
            success=True, reason="ok", n_trades=8,
            metrics={"sharpe": 1.2, "sortino": 1.5, "total_return": Decimal("0.12"),
                     "final_value": Decimal("112000"), "max_drawdown": Decimal("0.08"),
                     "cagr": 0.12, "win_rate": 0.6},
        )
        insert_backtest_result(
            conn, run_id=run_id, signal_id=sig_id, strategy_id=sid, result=bt,
            hold_bars=5, fees=0.0003, slippage=0.0005, init_cash=Decimal("100000"), generation=0,
        )
    return sid
...
def _seed_child_gen1(db_path, run_id, child_sid, sharpe: float = 1.4):
    ...
    with connect(db_path) as conn, txn(conn):
        sig_id = insert_pattern_signal(
            conn, run_id=run_id, strategy_id=child_sid, symbol="RELIANCE",
            n_signals=8, first_date=None, last_date=None, generation=1,
        )
        bt = BacktestResult(
            symbol="RELIANCE", pattern_name="SMA_10x25_bullish",
            success=True, reason="ok", n_trades=12,
            metrics={"sharpe": sharpe, "sortino": sharpe * 1.2,
                     "total_return": Decimal("0.15"), "final_value": Decimal("115000"),
                     "max_drawdown": Decimal("0.07"), "cagr": 0.15, "win_rate": 0.65},
        )
        insert_backtest_result(
            conn, run_id=run_id, signal_id=sig_id, strategy_id=child_sid, result=bt,
            hold_bars=5, fees=0.0003, slippage=0.0005, init_cash=Decimal("100000"), generation=1,
        )
...
def test_ratchet_node_writes_experiment_rows(deps, db_path, tmp_path):
    ...
    with connect(db_path) as conn, txn(conn):
        child_sid = upsert_strategy(
            conn, name="SMA_10x25_bullish", family="indicator",
...
def test_ratchet_accepts_improved_child(deps, db_path, tmp_path):
    ...
    node({
        "run_id": run_id,
        "generation": 1,
        "mutations": [{"generation": 0, "parent_strategy_id": parent_sid,
                       "child_strategy_id": child_sid, "mutator": "param_delta",
                       "mutation_json": "{}"}],
    })
...
def test_ratchet_noop_on_generation_zero(deps, db_path):
    ...
    with connect(db_path) as conn:
        n = conn.execute("SELECT COUNT(*) c FROM experiments WHERE run_id=?", (run_id,)).fetchone()["c"]
    assert n == 0
